...
class ConfigurationSet implements Diagnostic
{
   /**
     * @var Translator
     */
    ...
    public function execute()
    {
        ...
        $label = $this->translator->translate('RebelMetrics_DiagnosticPrefix') .
          $this->translator->translate('RebelMetrics_IsConfigured');
        $check = new Rebel($settings);
        $setup = $check->isSetUp();
        if ($setup) {
            $status = DiagnosticResult::STATUS_OK;
            $comment = $this->translator->translate('RebelMetrics_IsConfiguredOk');
        } else {
            $status = DiagnosticResult::STATUS_ERROR;
            $comment = $this->translator->translate('RebelMetrics_IsConfiguredError');
        }
